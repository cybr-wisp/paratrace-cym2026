...
const PIPELINE = [
  ...
  ["04", "Classification", "scikit-learn", "The baseline routine evaluates Random Forest, Gradient Boosting and Logistic Regression with stratified 5-fold CV; the degradation routine uses the same fold structure to evaluate rewritten feature sets at each level."],
...
function EvaluationNote() {
  return <div className="protocol-audit">
    <div className="protocol-audit-label">EVALUATION PROTOCOL</div>
    <div className="protocol-audit-grid">
      <div><span>METHOD</span><strong>Stratified 5-fold CV</strong><p>All levels (L0 through L4) use the same fold structure. Each fold trains on L0 training rows and evaluates held-out rows at every rewrite level. No train-set leakage.</p></div>
      <div><span>STATISTICAL TESTS</span><strong>Wilcoxon + BH FDR</strong><p>Paired Wilcoxon signed-rank tests with Benjamini-Hochberg false discovery rate correction across 20 features per level-backend group.</p></div>
    </div>
  </div>;
}
...
export default function Research() {
  ...
  return <div className="research-page">
    ...
    {tab === "methodology" && <div className="research-content">
      ...
      <section className="paper-section"><div className="paper-margin"><span>01.4</span><p>EVALUATION</p></div><div className="paper-body"><h2>Unified cross-validation across all levels.</h2><p className="lead">All levels (L0 through L4) use the same stratified 5-fold CV with identical fold assignments. Each fold trains on L0 training rows and evaluates held-out rows at every rewrite level. Wilcoxon signed-rank tests use Benjamini-Hochberg FDR correction across 20 features per level-backend group.</p></div></section>
    </div>}

    {tab === "results" && <div className="research-content">
      <section className="paper-section"><div className="paper-margin"><span>02.1</span><p>REPORTED SUMMARY</p></div><div className="paper-body"><h2>Rewriting approaches chance-level classification.</h2><p className="lead">Cross-validated accuracy drops from 73.4% at L0 to ~53% at L3/L4. Semantic similarity remains above 83%.</p><div className="research-chart-wide"><StudyAccuracyChart activeLevel={3} /></div><div className="result-table"><div className="result-head"><span>LEVEL</span><span>INTERVENTION</span><span>ANTHROPIC</span><span>OPENAI</span><span>AVERAGE</span></div>{STUDY_ACCURACY.map((row) => <div className={row.level >= 3 ? "result-row critical" : "result-row"} key={row.level}><span>L{row.level}</span><span>{row.label}</span><span>{row.anthropic.toFixed(1)}%</span><span>{row.openai.toFixed(1)}%</span><span><strong>{row.average.toFixed(1)}%</strong></span></div>)}</div><div className="finding-callout"><span>PROJECT CLAIM</span><p>The public repository summarizes the central phenomenon as a “what vs. how” gap: semantic content stays highly similar while the linguistic representation used for classification changes substantially.</p></div></div></section>

      <section className="paper-section"><div className="paper-margin"><span>02.2</span><p>PAIRED STATISTICS</p></div><div className="paper-body"><h2>At L2, 19 of 20 features are significant in each backend.</h2><p className="lead">The table below uses exact percentage-change values from the committed <code>statistical_tests.json</code>, not hand-entered “retention” estimates.</p><div className="evidence-matrix research-evidence"><div className="evidence-matrix-head"><span>FEATURE / L2 CHANGE FROM L0</span><span>ANTHROPIC</span><span>OPENAI</span></div>{L2_FEATURE_CHANGES.map((row) => <div className="evidence-matrix-row" key={row.name}><span>{row.name}</span><strong className={row.significantAnthropic ? "evidence-hot" : "evidence-neutral"}>{row.anthropic > 0 ? "+" : ""}{row.anthropic.toFixed(2)}%</strong><strong className={row.significantOpenAI ? "evidence-hot" : "evidence-neutral"}>{row.openai > 0 ? "+" : ""}{row.openai.toFixed(2)}%</strong></div>)}<div className="evidence-matrix-foot"><span>Incomplete-word rate is the non-significant feature in this L2 slice.</span><span>paired Wilcoxon · BH-corrected p &lt; 0.05</span></div></div><div className="method-note"><span>WHY NO SIMPLIFIED BRR HEATMAP HERE?</span><p>The repository defines BRR as |d<sub>level</sub>| / |d<sub>original</sub>|, so values can exceed 1 when group separation increases after rewriting. A decorative 0–1 “signal survival” heatmap would therefore misstate the artifact. This site uses exact paired-change values unless the full backend-specific BRR artifact is rendered directly.</p></div></div></section>
    </div>}
